...
import servicesImplementations.*;
...
public class GridImpl implements Grid{
	...
	private Point getLocationContainaible() {
		...
		while(iter <550)
		{
			...
			if(map.isReachable(in.x, in.y, p.x, p.y) && env.CellContent(p.x, p.y) == null && 
					...
					map.cellNature(p.x, p.y) != Cell.OUT
					&& map.cellNature(p.x, p.y) != Cell.IN) {
				return p;
			}
			iter++;
		}
	...
	public ArrayList<Point> getDoors(){
	 ...
	 ArrayList<Point> doorsKey;
	public void placedContainable(Player p1)
	{

		Point p=null;
		doorsKey =new ArrayList<>();  
	    ...
	    containers.add(p1);
		   ...
		   if(doors.size()!=0) {
		      for(int i= 0; i< doors.size(); i++) {
			    ...
			    if( (p=getLocationContainaible()) == null) {
		          ...
		          containers.add(key);
		          doorsKey.add(new Point(doors.get(i).x, doors.get(i).y));
			   }
			   }
		   }
		   Cow cow =new CowContract(new CowImpl());
		    ...
		    Treasure t = new TreasureImpl(); 
			Treasure tt = new TreasureContract(t);
			  if( (p=getLocationContainaible()) == null) {
				  containers.clear();
				   generatedGrid(p1);
				   return;}
	...
	public boolean checkFile(String file)
	{
	     boolean ff=true;		
			 try {
					...
					return comp == width*height && in==1 && out ==1;
	...
	public ArrayList<Key> keys() {
		...
		for(int i=0 ; i<map.width(); i++)
			for(int j=0;  j<map.height(); j++)
				if(env.CellContent(i, j)!=null && env.CellContent(i, j) instanceof Key)
					keys.add(( Key )env.CellContent(i, j));
		return keys;
	}
