...
public class MagicBeansTest {
	...
	@Test
	public void TestMagicBeansInitPositif() {
		
		//Tester les differents constructeurs ( pré et post )
	
		MagicBeans f = new MagicBeansContract(new MagicBeansImpl());
		f.init();
		assertTrue(f.Striking_power_bonus()==1);
		assertTrue(f.row()==0);
		assertTrue(f.col()==0);
		
		f.init(5);
		assertTrue(f.Striking_power_bonus()==5);
		assertTrue(f.row()==0);
		assertTrue(f.col()==0);	
	...
	@Test
	public void TestMagicBeansInitNegatif() {
